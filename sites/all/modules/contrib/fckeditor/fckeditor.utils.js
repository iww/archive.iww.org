    ...
    if (fckInstances[taid]) {
      ...
      if (editorInstance.defaultState == 1) {
        if (textarea.attr('class').indexOf("filterxss1") != -1 || textarea.attr('class').indexOf("filterxss2") != -1) {
          if (typeof Drupal.settings.fckeditor.settings[taid].input_format == 'undefined') {
            // 0 == FILTER_FORMAT_DEFAULT
            Drupal.settings.fckeditor.settings[taid].input_format = 0;
          }
          else if (typeof Drupal.settings.fckeditor.settings[taid].input_format == 'object') {
            Drupal.settings.fckeditor.settings[taid].input_format = Drupal.settings.fckeditor.settings[taid].input_format.pop();
          }
          $.post(Drupal.settings.basePath + 'index.php?q=fckeditor/xss', {
            'text': $('#' + taid).val(),
            'token': Drupal.settings.fckeditor.ajaxToken,
            'input_format': Drupal.settings.fckeditor.settings[taid].input_format
            },
            function(text) {
              textarea.val(text);
