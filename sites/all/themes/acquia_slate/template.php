...
function acquia_slate_preprocess_page(&$vars) {
  ...
  if (theme_get_setting('breadcrumb_display') == 0) {
   ...
   if (theme_get_setting('preamble_display') == 1) {
    $vars['preamble'] = theme_get_setting('preamble_nid');  
  } 
   if (theme_get_setting('branch_name_display') == 1) {
    $vars['branch_name'] = theme_get_setting('branch_name_text');  
  }
